...
def menu(request):
   ...
   """
    user = models.User.objects.filter(username='hanshengjian').first()
    ...
    for per in permission_list:
        ...
        # if re.match(per['permission__url'],request.path_info):   #应对用这个
        # if re.match(per['permission__url'],"/jieke.html"):
        if re.match(per['permission__url'],request.path_info):   #测试 显示的url
            item['opened'] = True
        pid = item['parent_id']
